...
const server = new McpServer({ name: "hashcat", version: "1.0.0" });

function runHashcat(hashcatArgs: string[]): Promise<{ content: Array<{ type: "text"; text: string }> }> {
    return new Promise((resolve, reject) => {
        const proc = spawn(binaryPath, hashcatArgs);
        let output = "";
...
server.tool(
    "hashcat_crack",
    `hashcat MCP server — world's fastest password cracker supporting 300+ hash types.
...
Note: GPU acceleration is not available in Docker without --gpus flag. CPU cracking is slower but functional.`,
    {
        ...
        extra_args: z.array(z.string()).optional().describe("Additional hashcat arguments (e.g., ['--increment'] for incremental mask, ['--session', 'mysession'] to name the session)."),
    },
    async ({ hash_file, hash_type, attack_mode, wordlist, mask, rules_file, output_file, extra_args }) => {
        ...
        if (wordlist) hashcatArgs.push(wordlist);
        if (mask) hashcatArgs.push(mask);
        if (rules_file) hashcatArgs.push("-r", rules_file);
        if (output_file) hashcatArgs.push("-o", output_file);
        if (extra_args) hashcatArgs.push(...extra_args);
...
server.tool(
    "hashcat_show",
    `hashcat MCP server — display previously cracked passwords from the potfile.
...
Always run hashcat_show after hashcat_crack to see the cracked results.`,
    {
        hash_file: z.string().describe("Path to the hash file used during cracking (e.g., /data/hashes.txt)."),
        hash_type: z.number().describe("Hash type code used during cracking (must match the -m value used in hashcat_crack)."),
    },
